...
class TodoListService {
    ...
    updateTodoList() {
        ...
        //complet 상태에 따라 아이콘을 다르게 표시(css에서 아이콘은 체크 상태에 따라 변경을 못해서)
...
`
    ...
    getTodoById(id) {
        ...
        console.log("gettodobyid : " + this.todoList.filter(todo => todo.id === parseInt(id))[0]) + "를 반환";
    ...
    selectActiveTodo(todoList) {
        const activeTodo = this.todoList.filter(todo => !todo.completStatus)
    ...
    selectCompletedTodo(todoList) {
        const CompletedTodo = this.todoList.filter(todo => {
